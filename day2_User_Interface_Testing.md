...
![newProject](./images/Day2-1.png)

이번 예제 프로젝트 셋업은 정말로 간단하지만, Xcode7 에서 UI 테스트가 어떻게 동작하는지를 설명하는 데는 충분하다.

![appLayout](./images/Day2-2.png)

스위치와 상세 뷰 컨트롤러를 연결하는 버튼을 포함하는 메뉴 뷰 컨트롤러가 있다. 스위치가 "off" 상태였을때, 버튼은 비활성화되어야 하며 네비게이션 기능은 불가능해야 한다. 상세 뷰 컨트롤러는 라벨의 값을 증가하는 간단한 버튼을 포함하고 있다.
...
![prerecording](./images/Day2-3.png)
...
이번 테스트는 사용자가 증가 버튼을 클릭했을 때 라벨의 값이 1씩 증가 하는지 확인하는 것이다. 이번 테스트의 처음 두 줄은 이전 테스트와 아주 비슷하기 때문에 이전 테스트에서 복사해서 붙여넣을 수 있다.
...
![accessibilityInspector](./images/Day2-4.png)
